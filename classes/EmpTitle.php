...
class EmpTitle
{
    ...
    {
        $query = 'SELECT et.id, et.`empId` AS empId, e.`empName` AS empName, jt.id AS titleId, jt.title AS title FROM employees_titles AS et LEFT JOIN employees AS e ON et.empId = e.id LEFT JOIN job_titles AS jt ON et.jobTitleId = jt.id';
